...
def total():
    ...
    try:
        ...
        cursor = cnx.cursor()
        query = ("select sum(no_of_trees) from data_storage")
...
def show():
    ...
    try:
        ...
        cnx = connection.MySQLConnection(user = 'root',
                                         ...
                                         database = 'carbon_footprints')
        cursor = cnx.cursor()
        query = ("select * data_storage")
        cursor.execute(query)

        for (s_no, time_of_measurement, data_stored_in_GB, co2_in_kgs, no_of_trees) in cursor:
            a = tk.Label(s, text = f'''{time_of_measurement} \n
                Data Stored or Transferred (GB) - {data_stored_in_GB} \n
...
def insert():
    ...
    def output():
        try:
            ...
            data_stored_in_GB = int(a2.get(1.0, "end-1c"))
            co2_in_kgs = (data_stored_in_GB)*3
            ...
            query = ('''insert into data_storage (data_stored_in_GB, co2_in_kgs, no_of_trees) 
            values (%f, %f, %f)''')
            data = (data_stored_in_GB, co2_in_kgs, no_of_trees)
    ...
    a1 = tk.Label(i, text = "Enter the Amount of Data Stored, Downloaded, or Transferred (GB):", font = ("Yu Gothic", 18, "bold"), fg = "#000000", bg = "#85FC91")
    a1.grid()
...
def delete():
    ...
    try:
        ...
        query = ('''delete from data_storage
